...
namespace WindowsProcess.Utilities
{
    ...
    public class EnvironmentBlock
    {
        ...
        public static EnvironmentBlock CreateForUser(NetworkCredential credential)
        {
            if (credential == null)
                throw new ArgumentNullException("credential");

            EnvironmentBlock envBlock;
            using (var token = credential.LogonAndGetUserPrimaryToken())
            {
                envBlock = CreateForUser(token.DangerousGetHandle());
            }
        ...
        private static IList<string> SplitDoubleNullTerminatedUniStrings(IntPtr ptrStrings)
        {
            ...
            do
            {
                ...
                current += Encoding.Unicode.GetByteCount(buffer) + Encoding.Unicode.GetByteCount("\0")
                    /* Null uni char */;
